...
CREATE TABLE IF NOT EXISTS `universityapp`.`university` (
  ...
  INDEX `univesity_rectors_idx` (`rector_id` ASC),
  INDEX `univesity_first_prorector_idx` (`first_prorector_id` ASC),
  INDEX `univesity_second_prorector_idx` (`second_prorector_id` ASC),
  CONSTRAINT `univesity_rector`
    FOREIGN KEY (`rector_id`)
    REFERENCES `universityapp`.`professor` (`professor_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `univesity_first_prorector`
    FOREIGN KEY (`first_prorector_id`)
    REFERENCES `universityapp`.`professor` (`professor_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `univesity_second_prorector`
